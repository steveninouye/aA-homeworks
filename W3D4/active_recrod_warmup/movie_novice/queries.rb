...
def below_average_years
  ...
  Movie.select(:yr, 'COUNT(*) AS bad_movies').where('score < 5').group(:yr).order('COUNT(*) desc')
end
...
def pulp_fiction_actors
  ...
  Actor.select("actors.id,actors.name").joins(:movies).where("movies.title LIKE '%Pulp Fiction%'")
end
...
def uma_movies
  ...
  Movie.select("movies.id,movies.title,movies.yr").joins(:actors).where("actors.name LIKE '%Uma Thurman%'").order('movies.yr ASC')
end
